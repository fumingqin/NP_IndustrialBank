		...
		methods: {
			...
			loadData: function() {
				...
				uni.request({
					...
					method: this.$ky_cpdg.KyInterface.Cs_getByTitle.method,
					success: (res) => {
						console.log('特惠须知', res)
						var data = res.data.data.filter(item => {
							return item.Type == '隐私政策';
						})
						this.way2 = data[0].Body;
						console.log('特惠须知2', this.way2)
					}
				})
			...
			queryClick: function() {
				...
				if (that.departure == '请选择起点') {
					...
					var params = '../../pages_ZXGP/pages/ZXGP/TraditionSpecial/Order/selectTickets?&startStation=' + this.departure + '&endStation=' + this
			...
			quick: function(item) {
				var params = '../../pages_ZXGP/pages/ZXGP/TraditionSpecial/Order/selectTickets?&startStation=' + item.StartSite + '&endStation=' + item.EndSite +
